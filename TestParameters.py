...
def setForcedObservations(mdp, nPerms=10, sPerms = 10):
    ...
    ranPerms = np.array([np.random.permutation(mdp.nP)
    ...
    sPIn = sPIn[:sPerms]
    sucPerms = V[sPIn,:]
    state = np.zeros((mdp.nT,nPerms,nV,2), dtype = int)
...
def runManyTrials(newActionPairs = False, apPriors = [],
    ...
    if newGoals == True:
        ...
        actf.C = setPriorGoals(mdp, goalShape, rampSlope, Gmean, Gscale)
    ...
    if forceObs == True:
        ...
        fstate = np.reshape(fstate,(s1,s2*s3*s4),order='F')
        fstate = fstate.T
    ...
    abel = np.zeros((nPerms,T,actf.Ns),dtype=float)
    aact = np.zeros((nPerms,T),dtype=int)
    aobs = np.zeros((nPerms,T),dtype=int)
    print 'Check 1: made it to the first for loop \n'
    sys.stdout.flush()
    for cper in xrange(nPerms):
        ...
        if forceObs == True:
            for t,st in enumerate(fstate[cper,:]):
                obs[t] = actf.sampleNextObservation(st)
