...
class KollingAgent(object):
    """Implements the agent as presented in Kolling_2014."""
    ...
    def find_optimal_risk_bonus(self, trial, thres, cpoints, cpair):
        ...
        for risk_bonus in np.arange(0, 1.1, 0.1):
            ...
            # it.permutations(apt_left,r = (self.nT - trial)):
            for ap_seq in [apt_left]:
                npoints = cpoints
...
#            self.all_orbs[(trial, cpair)] = opt_rbs
        ...
        vL = self.pL[cpair] * self.rL[cpair] + opt_rbs * \
            (1 - self.pL[cpair]) * self.rL[cpair]
        vH = self.pH[cpair] * self.rH[cpair] + opt_rbs * \
            (1 - self.pH[cpair]) * self.rH[cpair]
